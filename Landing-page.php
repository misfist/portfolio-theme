...
<!-- start welcome text -->
<?php if( get_option( 'cta_background_image_setting' != "" ,  get_template_directory_uri(). '/assets/images/block1.jpg')): ?>
...
<section class="welcome-text" >
	...
	<a href="<?php echo do_shortcode(esc_url(get_option('cta_button_href', '##'))); ?>"> <button><?php echo do_shortcode(esc_attr(get_option( 'cta-button-text' , esc_html__('Learn More', 'tar')))); ?></button></a>
</section>
...
<section class="portfolio">
	      ...
	      <?php $cat_id = intval( get_option( 'cats_elect', 1 ) ); ?>
				...
				<?php if( post_type_exists( 'jetpack-portfolio' ) ) : ?>
					<?php $tarPosts = new WP_Query( 'post_type=jetpack-portfolio&posts_per_page=6' ); ?>
				<?php else : ?>
					<?php $tarPosts = new WP_Query('cat='.$cat_id.'&posts_per_page=6'); ?>
				<?php endif; ?>
				       ...
				       <?php while($tarPosts->have_posts()) : $tarPosts->the_post(); ?>
				          <div class="portfolio-image">
								...
								<?php } ?>
						          	<a href="<?php the_permalink(); ?>" class="image-section"> <?php the_title(); ?>
								  <?php the_excerpt(); ?>
							  </a>
